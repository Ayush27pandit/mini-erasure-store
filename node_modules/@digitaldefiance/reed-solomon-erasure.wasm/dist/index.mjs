 ...
 */
async function loadWasmBytesNode(wasmPath) {
 ...
 */
function loadWasmBytesSyncNode(wasmPath) {
...
export class ReedSolomonErasure {
     ...
     */
    static async getInstance(wasmUrl) {
        if (cachedInstance) {
        ...
        if (wasmUrl) {
            if (isBrowser) {
                ...
                loadingPromise = loadWasmBytesNode(wasmUrl).then(bytes => ReedSolomonErasure.fromBytes(bytes));
            }
        }
        else {
     ...
     * ```
     */
    static fromCurrentDirectorySync() {
        if (isBrowser) {
            throw new Error('fromCurrentDirectorySync() is not available in browser. ' +
